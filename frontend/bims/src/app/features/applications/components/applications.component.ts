...
export class ApplicationsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.queryParamMap.subscribe(params => {
      if (params.get('new') === '1' && this.auth.isCustomer()) {
        this.openWizard();
  ...
  refresh() {
    ...
    if (this.auth.isCustomer()) this.bizSvc.getUsersBusinesses().subscribe(b => this.myBusinesses.set(b || []));
    else if (this.auth.isAdmin()) this.userSvc.getByRole('UNDERWRITER').subscribe(u => this.underwriters.set(u));
  ...
  openWizard(app?: Application) {
    if (this.auth.isCustomer() && this.myBusinesses().length === 0) {
      this.svc.error.set('No business profile found. Please create one first.');
