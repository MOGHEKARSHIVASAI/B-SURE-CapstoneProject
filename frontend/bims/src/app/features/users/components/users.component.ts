...
export class UsersComponent implements OnInit {
  ...
  create() {
    ...
    this.svc.create(this.form).subscribe({
      ...
      error: (e) => { 
        this.createError.set(e.error?.message || 'Error occurred while creating user.'); 
        this.saving.set(false); 
        setTimeout(() => this.createError.set(''), 8000);
      }
    });
  }
  ...
  deactivate(id: number) {
    ...
    this.svc.deactivate(id).subscribe({
      next: () => this.load(),
      error: (e) => {
        this.actionError.set(e.error?.message || e.error?.error || 'Failed to deactivate user.');
        setTimeout(() => this.actionError.set(''), 8000);
      }
    });
  }
  ...
  activate(id: number) {
    this.svc.activate(id).subscribe({
      next: () => this.load(),
      error: (e) => {
        this.actionError.set(e.error?.message || e.error?.error || 'Failed to activate user.');
        setTimeout(() => this.actionError.set(''), 8000);
      }
    });
  }
  ...
  resetPassword() {
    this.svc.resetPassword(this.resetUser()!.id, this.newPassword).subscribe({
      next: () => {
        this.resetMsg.set('Password reset successfully!');
        setTimeout(() => {
          this.resetMsg.set('');
          if (this.showReset()) this.showReset.set(false);
        }, 3000);
      },
      error: () => {
        this.resetMsg.set('Error resetting password');
        setTimeout(() => this.resetMsg.set(''), 6000);
      }
    });
  }
