...
export class ClaimsComponent implements OnInit {
  ...
  openFileWizard() {
    if (this.myPolicies().length === 0) {
      this.svc.error.set('You do not have any active policies to file a claim against. Please ensure you have an active policy first.');
      return;
    }
    this.fileForm = {
  ...
  saveDraftAndNext() {
    ...
    if (!this.fileForm.claimedAmount || +this.fileForm.claimedAmount <= 0) { this.fileError.set('Please enter a valid claimed amount greater than zero.'); return; }
    if (!this.fileForm.incidentDate) { this.fileError.set('Please select the incident date.'); return; }
    if (new Date(this.fileForm.incidentDate) > new Date()) { this.fileError.set('Incident date cannot be in the future.'); return; }
    if (!this.fileForm.incidentDescription?.trim() || this.fileForm.incidentDescription.trim().length < 20) {
      this.fileError.set('Please provide a detailed incident description (at least 20 characters).');
      return;
    }

    // Coerce policyId to number (Angular binds select values as strings)
    const payload = { ...this.fileForm, policyId: +this.fileForm.policyId };

    const obs$ = this.savedClaimId()
      ? this.svc.update(this.savedClaimId()!, payload)
      : this.svc.file(payload);
    ...
    obs$.subscribe({
      ...
      error: (e) => {
        this.fileError.set(this.formatApiError(e, 'Failed to save claim. Please try again.'));
        this.svc.loading.set(false);
      }
    });
  }
  ...
  submitFromWizard() {
    if (!this.savedClaimId()) {
      this.fileError.set('Please complete step 1 first.');
      return;
    }
    if (this.docCountForDraft() === 0) {
      // Warn but allow submission – officer can request docs later
      if (!confirm('No documents uploaded. Claims without supporting evidence may be rejected. Continue anyway?')) {
        return;
      }
    }
    this.svc.executeAction(this.svc.submit(this.savedClaimId()!), 'Claim submitted successfully! Our team will review it shortly.', () => {
      this.closeWizard();
    });
  }
