...
@Component({
    ...
    <div class="doc-list-container">
      @if (loading()) {
                    ...
                    <tr class="border-t border-stone-100 hover:bg-stone-50 transition-colors">
                      ...
                      <td class="p-3 text-right">
                        ...
                        @if (doc.extractedData) {
                          <button class="btn btn-ghost btn-sm text-indigo-600" (click)="viewData(doc)" title="AI Extracted Data">
        ...
        <!-- AI Extracted Features Viewer -->
        @if (selectedDoc()) {
          <div class="mt-4 p-4 rounded-xl bg-indigo-50 border border-indigo-100 animate-in fade-in slide-in-from-top-2 duration-300">
            <div class="flex justify-between items-center mb-3">
              <h4 class="text-xs font-bold text-indigo-900 flex items-center gap-2">
                <span class="material-symbols-outlined text-sm">psychology</span>
                AI Extracted Features: {{ selectedDoc()?.fileName }}
              </h4>
              <button class="text-indigo-400 hover:text-indigo-600" (click)="selectedDoc.set(null)">
                <span class="material-symbols-outlined text-sm">close</span>
              </button>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              @for (feature of getParsedFeatures(selectedDoc()); track feature.key) {
                <div class="bg-white p-2.5 rounded-lg border border-indigo-50 shadow-sm">
                  <div class="text-[10px] font-bold text-indigo-300 uppercase tracking-tight">{{ feature.key }}</div>
                  <div class="text-sm text-indigo-900 font-medium">{{ feature.value }}</div>
                </div>
              }
            </div>
          </div>
...
export class DocumentListComponent implements OnInit {
    ...
    getParsedFeatures(doc: Doc | null): { key: string, value: string }[] {
        if (!doc?.extractedData) return [];
        try {
            const data = JSON.parse(doc.extractedData);
            return Object.entries(data).map(([key, value]) => ({ 
                key: key.replace(/_/g, ' ').replace(/([A-Z])/g, ' $1').trim(), // Pretty key
                value: typeof value === 'object' ? JSON.stringify(value) : String(value) 
            }));
        } catch (e) {
            return [{ key: 'Raw Data', value: doc.extractedData }];
        }
    }
}
