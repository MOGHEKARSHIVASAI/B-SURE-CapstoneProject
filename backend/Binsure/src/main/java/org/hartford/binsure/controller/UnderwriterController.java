...
public class UnderwriterController {
        ...
        // GET /api/v1/underwriting/assigned
        // UNDERWRITER gets ALL applications assigned to them (not just pending)
        @GetMapping("/assigned")
        @PreAuthorize("hasRole('UNDERWRITER')")
        @Operation(summary = "Get all assigned applications (UNDERWRITER)", description = "Returns all applications ever assigned to the current underwriter, regardless of status.")
        public ResponseEntity<List<PolicyApplicationResponse>> getAssignedApplications() {
                Long underwriterId = securityUtils.getCurrentUserId();
                return ResponseEntity.ok(underwriterService.getApplicationsAssignedToUnderwriter(underwriterId).stream()
                                .map(entityMapper::toDto).collect(Collectors.toList()));
        }
}
