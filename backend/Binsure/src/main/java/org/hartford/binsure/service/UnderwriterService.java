...
public class UnderwriterService {
        ...
        public Integer calculateRiskScore(Long applicationId) {
                ...
                // Return existing score if already calculated to ensure consistency on reload
                if (application.getAiRiskScore() != null && application.getAiRiskScore() > 0) {
                    return application.getAiRiskScore();
                }

                // Trigger AI risk assessment if no score exists
                aiService.assessUnderwritingRisk(application);
                applicationRepository.save(application);
                
                return application.getAiRiskScore() != null ? application.getAiRiskScore() : 50;
        }
}
